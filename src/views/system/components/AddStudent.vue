...
import { addStudent } from "@/api/System/userManage";
export default {
    ...
    async submitForm(formName) {
      this.$refs[formName].validate(async (valid) => {
        if (valid && this.optionValue.length === 5) {
          this.formData.university_id = this.optionValue[0];
          this.formData.school_id = this.optionValue[1];
          this.formData.department_id = this.optionValue[2];
          this.formData.major_id = this.optionValue[3];
          this.formData.class_id = this.optionValue[4];
          try {
