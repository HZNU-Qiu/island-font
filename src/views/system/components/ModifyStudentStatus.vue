<template>
  <div class="container">
    <div class="title">
      <span>更改{{ this.oldData.studentName }}的学籍</span>
...
import { modifyStudentStatus, changeStudentAvatar } from "@/api/System/userManage";
export default {
  name: "ModifyStudentStatus",
  props: {
    oldData: Object,
  },
  data() {
  ...
  async mounted() {
    ...
    this.optionValue = [
      ...
      this.oldData.majorID,
      this.oldData.classID,
    ];
    ...
    async submitForm(formName) {
      this.$refs[formName].validate(async (valid) => {
        if (valid && this.optionValue.length === 5) {
          ...
          try {
            await modifyStudentStatus(this.formData);
            this.$emit("editClose");
            this.$message({
              type: "success",
              message: "添加成功",
