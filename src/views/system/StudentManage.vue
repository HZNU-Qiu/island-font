<template>
  <div>
    ...
    <!-- 学生创建窗口 -->
    <StudentAddition v-else-if="showEdit" @editClose="editClose"></StudentAddition>
    <!-- 学生学籍信息修改对话框 -->
    <ModifyStudentStatus v-if="showModify" @modifyClose="modifyClose" :oldData="oldData"></ModifyStudentStatus>
  </div>
</template>
...
import StudentAddition from "./components/AddStudent";
import ModifyStudentStatus from "./components/ModifyStudentStatus";
export default {
  components: {
    StudentAddition,
    ModifyStudentStatus,
  ...
  methods: {
    ...
    handleStatusModify(row) {
      this.oldData.studentName = row.realname
    ...
    handleCellSave(row) {
      ...
      modifyStudentInfo({ realname, sex, email, id }).then((res) => {
        this.$message({
          type: "success",
          message: "保存成功",
        });
